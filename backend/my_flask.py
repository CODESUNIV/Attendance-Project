...
def start_rasa():
    args = [
        "rasa",
        "run",  # Command to run the Rasa server
        "--model",
        "rasa-data/models/",  # Path to the trained model directory
        "--enable-api",  # Enable API endpoints
        "--cors",
        "*",  # Allow all CORS origins
        "--port",
        "5005",  # Specify the port (default is 5005)
        "--endpoints",
        "rasa-data/endpoints.yml",  # Path to the endpoints file (if applicable)
    ]
    ...
    print(f"Rasa server started in the background with PID: {process.pid}")
...
def login():
    try:
        ...
        return (
            jsonify({"success": False, "message": "Error occurred", "error": str(e)}),
            500,
        )
...
def get_students(course):
    try:
        ...
        if result:
            students = [
                {"name": row[0], "programName": row[1], "delayTime": row[2]}
                for row in result
            ]
...
def start_face_recognition():
    try:
        ...
        if not start_time or not accepted_delay:
            ...
            return (
                jsonify({"message": "Start time and accepted delay are required"}),
                400,
            )

        subprocess.Popen(
            ["python", "face.py", start_time, str(accepted_delay)],
...
def update_delay_time():
    try:
        ...
        cursor.execute(
            "UPDATE attendance SET delay_time = %s WHERE full_name = %s",
            (delay_time, student_name),
        )
        connection.commit()
...
def get_student_details(studentName):
    try:
        ...
        cursor.execute(
            "SELECT Name, studentNumber FROM students WHERE Name = %s", (studentName,)
        )
        ...
        if student_details:
            return (
                jsonify(
                    {"name": student_details[0], "studentNumber": student_details[1]}
                ),
                200,
            )
        else:
...
def adminadd_course():
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO courses (Course, Professor, ProgramName) VALUES (%s, %s, %s)",
            (course, professor, program_name),
        )
        conn.commit()
        conn.close()
...
def adminadd_student():
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO students (Name, Password, Program, StudentNumber) VALUES (%s, %s, %s, %s)",
...
def adminupdate_student(id):
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE students SET Name = %s, Password = %s, Program = %s, StudentNumber = %s WHERE ID = %s",
...
def adminadd_program():
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO programs (ProgramName) VALUES (%s)", (program_name,)
        )
        conn.commit()
        conn.close()
...
def adminupdate_program(id):
    try:
        ...
        program_name = data.get(
            "program_name"
        )  # Key should be 'program_name' as sent from frontend
        ...
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE programs SET ProgramName = %s WHERE ID = %s", (program_name, id)
        )
        conn.commit()
        conn.close()
    ...
    try:
        if "file" in request.files:
            ...
            if not file.filename.endswith((".xlsx", ".xls")):
                return (
                    jsonify(
                        {"error": "Invalid file format. Only Excel files are allowed."}
                    ),
                    400,
                )
        ...
        if "file" in request.files:
            ...
            if not all(col in df.columns for col in required_columns):
                return (
                    jsonify(
                        {
                            "error": "Missing required columns: Course, Professor, Program"
                        }
                    ),
                    400,
                )
            ...
            courses = [
                (row["Course"], row["Professor"], row["Program"])
            ...
            if not data:
                return jsonify({"error": "No data provided"}), 400
            courses = [
                (course["course"], course["professor"], course["program"])
                for course in data
            ]
